...
def build_predictions(mp: pd.DataFrame, teams_today: set[str]) -> pd.DataFrame:
    """
    ...
    todays_players["goal_probability"] = (
        todays_players["xg_per_game"] * (1 + (todays_players["is_pp1"] * pp1_boost))
    ).clip(upper=0.35)
...
def main() -> int:
    ...
    # Predictions-only
    pred = build_predictions(mp, teams_today)
    ...
    preview = pred.sort_values("goal_probability", ascending=False)[
        ["name", "team", "lambda_goal", "goal_probability", "I_F_goals", "games_played", "is_pp1"]
    ].head(args.top)
    ...
    if len(positive_ev) > 0:
        ...
        print(
            positive_ev[
                ["player", "team", "odds", "implied_prob", "goal_probability", "ev_percent", "is_pp1"]
            ]
            .head(20)
